...
#  number of cannibals to be moved from left to right or right to left for particular state
...
class Solution():
    ...
    def number_of_cannibals_exceeds(self, number_missionaries, number_cannibals):
        ...
        return (number_cannibals > number_missionaries and number_missionaries > 0) or \
               (number_cannibals_right > number_missionaries_right and number_missionaries_right > 0)
    ...
    def solve(self, solve_method="dfs"):
        ...
        self.visited[self.start_state] = None

        return self.dfs(*self.start_state, 0) if solve_method == "dfs" else self.bfs()

    def draw_legend(self):
        # Utility method to add legend on graph if legend flag is ON
    ...
    def show_solution(self):
        ...
        self.draw(number_missionaries_left=number_missionaries_left ,number_cannibals_left=number_cannibals_left, number_missionaries_right=number_cannibals_right, number_cannibals_right=number_cannibals_right)
        ...
        for i, ((number_missionaries, number_cannibals, side), node) in enumerate(zip(steps[1:], nodes[1:])):
            ...
            print(f"Step {i + 1}: Move {number_missionaries} missionaries and {number_cannibals} cannibals from {self.boat_side[side]} to {self.boat_side[int(not side)]}.")
