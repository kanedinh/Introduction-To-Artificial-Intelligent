...
def draw_edge(number_missionaries, number_cannibals, side, depth_level, node_num):
    ...
    if Parent[(number_missionaries, number_cannibals, side, depth_level, node_num)] is not None:
        ...
        edge = pydot.Edge(str(Parent[(number_missionaries, number_cannibals, side, depth_level, node_num)]),
                          str((number_missionaries, number_cannibals, side, depth_level, node_num)),
                          dir='forward')
        graph.add_edge(edge)
    else:
        # For start node
...
def number_of_cannibals_exceeds(number_missionaries, number_cannibals):
    ...
    return (number_cannibals > number_missionaries and number_missionaries > 0) \
        or (number_cannibals_right > number_missionaries_right and number_missionaries_right > 0)
...
def generate():
    ...
    while q:
        ...
        # print(f"Processing: {number_missionaries}, {number_cannibals}")
