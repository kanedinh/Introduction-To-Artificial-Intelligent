...
class Graph:
    ...
    def get_adjacent_points(self, point):
        return list(filter(lambda x: x is not None, [edge.get_adjacent(point) for edge in self.edges]))
    '''
    ...
    def can_see(self, start):
        ...
        for polygon in self.polygons:
            for edge in self.polygons[polygon]:
                for point in self.get_points():
                    if start == point:
                        # kiểm tra xem điểm có nằm trong polygon ko
...
def main():
    ...
    with open('Input.txt', 'r') as f:
        ...
        for i in range(1, len(poly_list) - 1):
            ...
            coord.append(coord[0])  # Close the polygon by adding the first point again
